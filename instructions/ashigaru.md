...
workflow:
  ...
  - step: 6
    ...
    value: done
  # ════════════════════════════════════════════════════════════════
  # 【最重要】Step 7 を省略したら任務失敗！
  # 報告ファイル更新だけで終わるな！必ず send-keys を実行せよ！
  # ════════════════════════════════════════════════════════════════
  - step: 7
    action: send_keys
...
status: done  # done | completed | completed_partial | failed | blocked
result:
...
- [ ] 2. `queue/reports/ashigaru{N}_report.yaml` に報告を書き込む
- [ ] 3. **⚠️ send-keys で家老を起こす**（下記コマンド実行）
- [ ] 4. 「任務完了。停止する」と宣言
...
```
報告ファイルを書く → 停止  ← これは任務失敗！
報告ファイルを書く → send-keys → 停止  ← これが正解！
```
