...
- **send-keys は `&&` で順次実行**（並列Bash呼び出し禁止、順序が保証されない）

### 報告の流れ（割り込み防止設計）

| 報告経路 | 方法 | 理由 |
|----------|------|------|
| 足軽 → 家老 | 報告YAML + **send-keys 必須** | 家老が報告を認識するため |
| 家老 → 将軍/殿 | dashboard.md 更新のみ（send-keys **禁止**） | 殿の入力中の割り込み防止 |

**上→下への指示**: YAML + send-keys で起こす

```
⚠️ 足軽は報告ファイル更新だけで終わるな！
   必ず send-keys で家老を起こせ！
```
...
足軽がタスクを完了し家老への報告が完了したタイミングで、家老は足軽に `/clear` を実行させる。
これにより足軽のコンテキストウィンドウをリセットし、疲弊を防ぐ。
...
```bash
# 足軽1に /clear を送信（&& で順次実行、並列禁止）
tmux send-keys -t multiagent:0.1 '/clear' && tmux send-keys -t multiagent:0.1 Enter
```

### 重要ルール

- **家老は足軽のタスク完了報告を受けたら、必ず `/clear` を送信せよ**
- **次のタスクを割り当てる前に `/clear` を実行すること**
...
- **足軽**: 基本は自身でBash実行。**Git操作は例外的にサブエージェントに委譲**
  - `git-status` → 状態確認（diffの大量出力を回避）
  - `git-commit` → コミット作成
  - `git-push` → プッシュ
